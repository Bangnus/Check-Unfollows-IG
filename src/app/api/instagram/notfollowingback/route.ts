...
async function loginInstagram(page: Page, username: string, password: string): Promise<boolean> {
    try {
        ...
        await page.type(usernameSelector, username, { delay: Math.random() * 200 + 100 });

        console.log("⌨️ Typing password...");
        const passwordSelector = 'input[name="password"]';
        await page.waitForSelector(passwordSelector, { timeout: 10000 });
        await page.type(passwordSelector, password, { delay: Math.random() * 200 + 100 });
        ...
        if (pageContent.includes("Sorry, your password was incorrect") ||
            ...
            console.warn("❌ Login failed: Incorrect password or suspicious attempt.");
            return false;
        }
...
async function scrapeUsersFromDialog(page: Page): Promise<InstagramUser[]> {
    ...
    while (attempts < maxAttempts) {
        ...
        const users = await page.evaluate((existingUsernames) => {
            ...
            links.forEach(link => {
                ...
                if (!foundUsernames.has(username)) {
                    ...
                    const img = link.querySelector("img") || link.closest("div[role='button']")?.querySelector("img");
                    ...
                    if (parentRow) {
                        ...
                        fullName = texts.find(t => t !== username && t !== 'Follow' && t !== 'Remove') || "";
                    }
        ...
        const scrollSuccess = await page.evaluate(async () => {
            ...
            let scrollContainer = document.querySelector('.custom-scroll-target') as HTMLElement;
            ...
            if (!scrollContainer) {
                 ...
                 if (dialog) {
                     ...
                     scrollContainer = divs.reduce((max, cur) => cur.scrollHeight > max.scrollHeight ? cur : max, divs[0]) as HTMLElement;
                 }
            }
...
async function scrapeUserList(page: Page, type: "following" | "followers", clientuser: string): Promise<InstagramUser[]> {
    try {
        ...
        await page.goto(`https://www.instagram.com/${clientuser}/`, { waitUntil: "networkidle2", timeout: 60000 });
...
export const POST = async (req: NextRequest) => {
    try {
        ...
        if (username && password) {
            ...
            if (!page || page.isClosed()) {
                return NextResponse.json(
                    { error: "Session expired and no credentials provided. Please provide username and password." },
                    { status: 401 }
                );
            }
        }
